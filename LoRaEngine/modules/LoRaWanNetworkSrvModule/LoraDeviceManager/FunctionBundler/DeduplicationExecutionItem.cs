...
namespace LoraDeviceManager.FunctionBundler
{
    ...
    public class DeduplicationExecutionItem : IFunctionBundlerExecutionItem
    {
            ...
            TelemetryConfiguration telemetryConfiguration)
        {
            ...
            var metricIdentifier = new MetricIdentifier(LoraDeviceManagerConstants.MetricNamespace, ConnectionOwnershipChangeMetricName);
            connectionOwnershipChangedMetric = telemetryClient.GetMetric(metricIdentifier);
        }
        ...
        internal async Task<DeduplicationResult> GetDuplicateMessageResultAsync(DevEui devEUI, string gatewayId, uint clientFCntUp, uint clientFCntDown, ILogger logger = null)
        {
            using var cts = new CancellationTokenSource(DuplicateMessageTimeout);
            var isDuplicate = true;
            var processedDevice = gatewayId;

            using (var deviceCache = new LoRaDeviceCache(cacheStore, devEUI, gatewayId))
            {
                if (await deviceCache.TryToLockAsync())
                {
                    if (deviceCache.TryGetInfo(out var cachedDeviceState))
                    {
                        ...
                        if (updateCacheState)
                        {
                            ...
                            {
                                connectionOwnershipChangedMetric.TrackValue(1);
                                ...
                                var method = new CloudToDeviceMethod(LoraDeviceManagerConstants.CloudToDeviceCloseConnection, TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(1));
                                var jsonContents = JsonConvert.SerializeObject(loraC2DMessage);
                                _ = method.SetPayloadJson(jsonContents);

                                try
                                {
                                    if (await edgeDeviceGetter.IsEdgeDeviceAsync(previousGateway, cts.Token))
                                    {
                                        var res = await serviceClient.InvokeDeviceMethodAsync(previousGateway, Constants.NetworkServerModuleId, method, default);
                                        ...
                                        if (res is null || res is { } && !HttpStatusCodeUtilities.IsSuccessStatusCode(res.Status))
                                        {
                                    ...
                                    {
                                        await channelPublisher.PublishAsync(previousGateway, new LnsRemoteCall(RemoteCallKind.CloseConnection, jsonContents));
